...
palindrome_found:
	...
	MOV		r0, #0				// letter offset
	LDR 	r3, =0xFF201000 	// JTAG UART base address
	
	// load string
	LDR		r1, =uart_palindrome_found

	
	BL palindrome_found_uart_loop

palindrome_found_uart_loop:

	LDRB	r2, [r1, r0]		// load letter from string
	
	CMP 	r2, #0x00
	BLEQ	exit				// exit if null byte
	
	//LDR 	r4, [r3, #4] 		// read the JTAG UART control register
	//LDR		r5, =0xFFFF
	//ANDS	r4, r4, r5			// check for write space
	//BEQ		.+8	
	STR		r2, [r3]			// write letter
	
	ADD 	r0, r0, #1			// increment letter iterator
	
	B 		palindrome_found_uart_loop
...
palindrome_not_found:
	...
	// load string
	LDR		r1, =uart_palindrome_not_found
	
	BL 		palindrome_not_found_uart_loop

palindrome_not_found_uart_loop:	
...
.align
	...
	uart_palindrome_found: 		.asciz "Palindrome detected\n"
	uart_palindrome_not_found:	.asciz "Not a palindrome\n"
.end
